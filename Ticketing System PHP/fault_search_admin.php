...
if(isset($_POST["submit"]))                                     // Checks if we got here by submitting the form for searching
{
    ...
    $title = $_POST["stitle"];
    $location = $_POST["slocation"];                                // Grabs the variables we got by submitting the form
    ...
    {
        header("Location: search_fault_form.php?search=nosearch");  // If not redirects and throws a message
        exit;
    }
    ...
    try 
    {
        ...
        {                                                                                   // It stores the collumns in the $row array
            ...
            echo '<td><a href="edit_fault_form.php?ID='.$row['ID'].'" class="button">Edit Fault</a></td>';  // Edit button with a relevant link for functionality
            echo '<td><a href="delete_faults.php?ID='.$row['ID'].'" class="dbutton" onclick="return confirm(\'Are you sure?\');">Delete Fault</a></td>'; // Delete button with a relevant link for functionality 
            echo '</tr>';
        }
